...
<p>The <code class='csharp'>OrderResponseErrorCode.None</code><code class='python'>OrderResponseErrorCode.NONE</code> (0) error means there is no order response error.</p>


<h4>Processing Error</h4>
<p>The <code class='csharp'>OrderResponseErrorCode.ProcessingError</code><code class='python'>OrderResponseErrorCode.PROCESSING_ERROR</code> (-1) error occurs in the following situations:</p>
<ul>
...
<p>The <code class='csharp'>OrderResponseErrorCode.OrderAlreadyExists</code><code class='python'>OrderResponseErrorCode.ORDER_ALREADY_EXISTS</code> (-2) error occurs when you submit a new order but you already have an open order or a completed order with the same order ID. This order response error usually comes from a concurrency issue.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.BrokerageModelRefusedToSubmitOrder</code><code class='python'>OrderResponseErrorCode.BROKERAGE_MODEL_REFUSED_TO_SUBMIT_ORDER</code> (-4) error occurs when you place an order but the <a href='/docs/v2/writing-algorithms/reality-modeling/brokerages/key-concepts'>brokerage model</a> determines it's invalid. The brokerage model usually checks your order meets the following requirements before sending it to the brokerage:</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.BrokerageFailedToSubmitOrder</code><code class='python'>OrderResponseErrorCode.BROKERAGE_FAILED_TO_SUBMIT_ORDER</code> (-5) error occurs when you place an order but the brokerage implementation fails to submit the order to your brokerage.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.BrokerageFailedToUpdateOrder</code><code class='python'>OrderResponseErrorCode.BROKERAGE_FAILED_TO_UPDATE_ORDER</code> (-6) error occurs when you try to update an order but the brokerage implementation fails to submit the order update request to your brokerage.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.BrokerageFailedToCancelOrder</code><code class='python'>OrderResponseErrorCode.BROKERAGE_FAILED_TO_CANCEL_ORDER</code> (-8) error occurs when you try to cancel an order but the brokerage implementation fails to submit the cancel request to your brokerage.</p>    
...
<p>The <code class='csharp'>OrderResponseErrorCode.UnableToFindOrder</code><code class='python'>OrderResponseErrorCode.UNABLE_TO_FIND_ORDER</code> (-10) error occurs when you try to place, update, or cancel an order, but the <code>BrokerageTransactionHandler</code> doesn't have a record of the order ID.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.UnsupportedRequestType</code><code class='python'>OrderResponseErrorCode.UNSUPPORTED_REQUEST_TYPE</code> (-12) error occurs in the following situations:</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.MissingSecurity</code><code class='python'>OrderResponseErrorCode.MISSING_SECURITY</code> (-14) error occurs when you place an order for a security but you don't have a subscription for the security in your algorithm.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.ExchangeNotOpen</code><code class='python'>OrderResponseErrorCode.EXCHANGE_NOT_OPEN</code> (-15) error occurs in the following situations:</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.SecurityPriceZero</code><code class='python'>OrderResponseErrorCode.SECURITY_PRICE_ZERO</code> (-16) error occurs when you place an order or exercise an Option contract while the security price is $0. The security price can be $0 for the following reasons:</p>
...
<p><a id='forex-conversion-rate-zero'></a>The <code class='csharp'>OrderResponseErrorCode.ForexBaseAndQuoteCurrenciesRequired</code><code class='python'>OrderResponseErrorCode.FOREX_BASE_AND_QUOTE_CURRENCIES_REQUIRED</code> (-17) error occurs when you place a trade for a Forex or Crypto pair but you don't have the base currency and <a href='/docs/v2/writing-algorithms/trading-and-orders/key-concepts#06-Quote-Currency'>quote currency</a> in your <a href='/docs/v2/writing-algorithms/portfolio/cashbook'>cash book</a>. This error should never occur. If it does, create a bug report.</p>


<h4>Forex Conversion Rate Zero</h4>
<p><a id='security-has-no-data'></a>The <code class='csharp'>OrderResponseErrorCode.ForexConversionRateZero</code><code class='python'>OrderResponseErrorCode.FOREX_CONVERSION_RATE_ZERO</code> (-18) error occurs when you place a trade for a Forex or Crypto pair and LEAN can't convert the value of the base currency to your account currency. This error usually indicates a lack of data. Investigate the data and if there is some missing, <a href='/docs/v2/cloud-platform/datasets/data-issues#04-Report-New-Issues'>report it</a>.</p>


<h4>Security Has No Data</h4>
<p><a id='exceeded-maximum-orders'></a>The <code class='csharp'>OrderResponseErrorCode.SecurityHasNoData</code><code class='python'>OrderResponseErrorCode.SECURITY_HAS_NO_DATA</code> (-19) error occurs when you place an order for a security before your algorithm receives any data for it. <?php echo file_get_contents(DOCS_RESOURCES."/initialization/zero-price-error.html"); ?>




<h4>Exceeded Maximum Orders</h4>
<p>The <code class='csharp'>OrderResponseErrorCode.ExceededMaximumOrders</code><code class='python'>OrderResponseErrorCode.EXCEEDED_MAXIMUM_ORDERS</code> (-20) error occurs when exceed your order quota in a backtest. The number of orders you can place in a single backtest depends on the tier of your <a href='/docs/v2/cloud-platform/organizations/getting-started'>organization</a>. The following table shows the number of orders you can place on each tier:</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.MarketOnCloseOrderTooLate</code><code class='python'>OrderResponseErrorCode.MARKET_ON_CLOSE_ORDER_TOO_LATE</code> (-21) error occurs when you try to place a market on close (MOC) order too early in the trading day.</p>
...
<p><a id='request-canceled'></a>The <code class='csharp'>OrderResponseErrorCode.InvalidRequest</code><code class='python'>OrderResponseErrorCode.INVALID_REQUEST</code> (-22) error occurs when you try to cancel an order multiple times.</p>

<p>To avoid this order response error, only try to cancel an order one time.</p>


<h4>Request Canceled</h4>
<p><a id='algorithm-warming-up'></a>The <code class='csharp'>OrderResponseErrorCode.RequestCanceled</code><code class='python'>OrderResponseErrorCode.REQUEST_CANCELED</code> (-23) error occurs when you try to cancel an order multiple times.</p>

<p>To avoid this order response error, only try to cancel an order one time.</p>


<h4>Algorithm Warming Up</h4>
<p>The <code class='csharp'>OrderResponseErrorCode.AlgorithmWarmingUp</code><code class='python'>OrderResponseErrorCode.ALGORITHM_WARMING_UP</code> (-24) error occurs in the following situations:</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.BrokerageModelRefusedToUpdateOrder</code><code class='python'>OrderResponseErrorCode.BROKERAGE_MODEL_REFUSED_TO_UPDATE_ORDER</code> (-25) error occurs in backtests when you try to update an order in a way that the <a href='/docs/v2/writing-algorithms/reality-modeling/brokerages/key-concepts'>brokerage model</a> doesn't support.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.InvalidNewOrderStatus</code><code class='python'>OrderResponseErrorCode.INVALID_NEW_ORDER_STATUS</code> (-32) error occurs in live trading when you try to update or cancel an order while it still has <code class="csharp">OrderStatus.New</code><code class="python">OrderStatus.NEW</code> status.</p>

<p>To avoid this order response error, check the <code class="csharp">Status</code><code class="python">status</code> property of the <a href='/docs/v2/writing-algorithms/trading-and-orders/order-management/order-tickets'>order ticket</a> or <a href='/docs/v2/writing-algorithms/trading-and-orders/order-events'>order event</a> before you update or cancel an order.</p>
...
<p>The <code class='csharp'>OrderResponseErrorCode.EuropeanOptionNotExpiredOnExercise</code><code class='python'>OrderResponseErrorCode.EUROPEAN_OPTION_NOT_EXPIRED_ON_EXERCISE</code> (-33) error occurs when you try to exercise a European Option contract before its expiry date.</p>
