...
consolidator = IdentityDataConsolidator[TradeBar]()   # Identity consolidator that passes through each TradeBar data point
...
consolidator = new IdentityDataConsolidator&lt;TradeBar&gt;();   // Identity consolidator that passes through each TradeBar data point

// Register the indicator to update with the consolidated data
RegisterIndicator(_symbol, _indicator, consolidator);</pre>
</div>

<p>Data point indicators use only a single price data in their calculations. By default, those indicators use the closing price. For assets with <code>TradeBar</code> data, that price is the <code>TradeBar</code> close price. For assets with <code>QuoteBar</code> data, that price is the mid-price of the bid closing price and the ask closing price. To create an indicator with the other fields like the <code class='csharp'>Open</code><code class='python'>open</code>, <code class='csharp'>High</code><code class='python'>high</code>, <code class='csharp'>Low</code><code class='python'>low</code>, or <code class='csharp'>Close</code><code class='python'>close</code>, provide a <code>selector</code> argument to the <code class="csharp">RegisterIndicator</code><code class="python">register_indicator</code> method.</p><div class="section-example-container">
