...
<pre class="python">class RiseFallThreeMethodsAlgorithm(QCAlgorithm):
    def initialize(self) -> None:
        ...
        indicator_history = self.indicator_history(_rise_fall_three_methods, self._symbol, 100, Resolution.MINUTE)
        timedelta_indicator_history = self.indicator_history(_rise_fall_three_methods, self._symbol, timedelta(days=10), Resolution.MINUTE)
        time_period_indicator_history = self.indicator_history(_rise_fall_three_methods, self._symbol, datetime(2024, 7, 1), datetime(2024, 7, 5), Resolution.MINUTE)</pre></div>