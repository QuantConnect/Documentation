...
<pre class="python">class UpsideGapTwoCrowsAlgorithm(QCAlgorithm):
    def initialize(self) -> None:
        ...
        indicator_history = self.indicator_history(_upside_gap_two_crows, self._symbol, 100, Resolution.MINUTE)
        timedelta_indicator_history = self.indicator_history(_upside_gap_two_crows, self._symbol, timedelta(days=10), Resolution.MINUTE)
        time_period_indicator_history = self.indicator_history(_upside_gap_two_crows, self._symbol, datetime(2024, 7, 1), datetime(2024, 7, 5), Resolution.MINUTE)</pre></div>