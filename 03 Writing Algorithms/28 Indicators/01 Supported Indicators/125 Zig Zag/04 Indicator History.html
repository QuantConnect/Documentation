...
<pre class="csharp">public class ZigZagAlgorithm : QCAlgorithm
{
    ...
    public override void Initialize()
    {
        ...
        var indicatorHistory = IndicatorHistory(_zz, _symbol, 100, Resolution.Minute);
        var timeSpanIndicatorHistory = IndicatorHistory(_zz, _symbol, TimeSpan.FromDays(10), Resolution.Minute);
        var timePeriodIndicatorHistory = IndicatorHistory(_zz, _symbol, new DateTime(2024, 7, 1), new DateTime(2024, 7, 5), Resolution.Minute);
...
<pre class="python">class ZigZagAlgorithm(QCAlgorithm):
    def initialize(self) -> None:
        ...
        indicator_history = self.indicator_history(self._zz, self._symbol, 100, Resolution.MINUTE)
        timedelta_indicator_history = self.indicator_history(self._zz, self._symbol, timedelta(days=10), Resolution.MINUTE)
        time_period_indicator_history = self.indicator_history(self._zz, self._symbol, datetime(2024, 7, 1), datetime(2024, 7, 5), Resolution.MINUTE)
    
        # Access all attributes of indicator_history
        indicator_history_df = indicator_history.data_frame
        high_pivot = indicator_history_df["high_pivot"]
        low_pivot = indicator_history_df["low_pivot"]</pre></div>