...
<pre class="csharp">public class AverageDirectionalMovementIndexRatingAlgorithm : QCAlgorithm
{
    ...
    public override void Initialize()
    {
        ...
        var indicatorHistory = IndicatorHistory(_adxr, _symbol, 100, Resolution.Minute);
        var timeSpanIndicatorHistory = IndicatorHistory(_adxr, _symbol, TimeSpan.FromDays(10), Resolution.Minute);
        var timePeriodIndicatorHistory = IndicatorHistory(_adxr, _symbol, new DateTime(2024, 7, 1), new DateTime(2024, 7, 5), Resolution.Minute);

        // Access all attributes of indicatorHistory
        var adx = indicatorHistory.Select(x => ((dynamic)x).Adx).ToList();
    }
}</pre>
<pre class="python">class AverageDirectionalMovementIndexRatingAlgorithm(QCAlgorithm):
    def initialize(self) -> None:
        ...
        indicator_history = self.indicator_history(self._adxr, self._symbol, 100, Resolution.MINUTE)
        timedelta_indicator_history = self.indicator_history(self._adxr, self._symbol, timedelta(days=10), Resolution.MINUTE)
        time_period_indicator_history = self.indicator_history(self._adxr, self._symbol, datetime(2024, 7, 1), datetime(2024, 7, 5), Resolution.MINUTE)
    
        # Access all attributes of indicator_history
        indicator_history_df = indicator_history.data_frame
        adx = indicator_history_df["adx"]</pre></div>