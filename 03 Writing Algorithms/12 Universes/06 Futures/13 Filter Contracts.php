<p>By default, LEAN doesn't add any contracts to the <a href="/docs/v2/writing-algorithms/securities/asset-classes/futures/handling-data#02-Futures-Chains">FuturesChain</a> it passes to the <code>OnData</code> method. To add a universe of Future contracts, in the <code>Initialize</code> method, call the <code>SetFilter</code> method of the <code>Future</code> object. The following table describes the available filter techniques:</p>
...
$getFilterCaveatText("Future");
?>

<p>By default, LEAN adds contracts to the <code>FutureChain</code> that pass the filter criteria at every time step in your algorithm. If a contract has been in the universe for a duration that matches the <code>MinimumTimeInUniverse</code> <a href='/docs/v2/writing-algorithms/universes/settings#02-Properties'>setting</a> and it no longer passes the filter criteria, LEAN removes it from the chain</p>
