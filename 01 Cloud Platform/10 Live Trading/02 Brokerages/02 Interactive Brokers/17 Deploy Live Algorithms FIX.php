...
$authentication = "
    ...
    <ol>
        ...
        <p>QuantConnect generates the <span class='field-name'>Operator ID</span> from the account you select, so it stays empty until you select an <span class='field-name'>Account ID</span>. Your IB account only accepts orders from QuantConnect after you register this value, so add it before you continue.</p>
        <p>To register it, follow the steps in <a href='/docs/v2/cloud-platform/live-trading/brokerages/interactive-brokers#03-FIX-Integration'>FIX Integration</a> and then return to this page. You only need to register each <span class='field-name'>Operator ID</span> once, so repeat those steps when you trade an account you haven't registered yet. After you save it, wait about 5 minutes for IB to apply the change before you deploy.</p>
    </ol>";
