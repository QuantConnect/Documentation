...
<p>The following table describes the available order types for each asset class that <?= $cloudPlatform ? "our Public integration" : "the <code>PublicBrokerageModel</code>" ?> supports:</p>

<table class="qc-table table" id='order-types-table'>
   <thead>
      <tr>
        <th style='width: 36%'>Order Type</th>
        <th style='width: 16%'>Equity</th>
        <th style='width: 16%'>Equity Options</th>
        <th style='width: 16%'>Index Options</th>
        <th style='width: 16%'>Crypto</th>
      </tr>
   </thead>
   <tbody>
      <tr>
...
<p><?=$writingAlgorithms ? "The <code>PublicBrokerageModel</code> supports custom order properties." : "We model custom order properties from the Public API." ?> The following table describes the members of the <code>PublicOrderProperties</code> object that you can set to customize order execution.</p>
...
<table class="table qc-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>If set to true, allows the order to trigger or fill outside of regular trading hours. This property applies to Equity limit orders with a day time-in-force only, since Public only accepts limit orders in the extended session.</td>
            <td><code class="csharp">false</code><code class="python">False</code></td>
        </tr>
        <tr>
            <td><code class="csharp">UseMargin</code><code class="python">use_margin</code></td>
            <td><code>bool?</code></td>
            <td>Controls the buying power the order uses. If set to true, the order uses margin buying power when the account allows it. If set to false, the order uses cash-only (settled cash) buying power. If you don't set it, orders on margin accounts use margin buying power and orders on cash accounts always use cash-only buying power.</td>
...
<h4>Updates</h4>
<p><?= $writingAlgorithms ? "The <code>PublicBrokerageModel</code> supports" : "We model the Public API by supporting" ?> <a href='/docs/v2/writing-algorithms/trading-and-orders/order-management/order-tickets#04-Update-Orders'>order updates</a> for single-leg orders. Public doesn't support updating combo (multi-leg) orders.</p>
