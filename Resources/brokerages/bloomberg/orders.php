<p>Both Bloomberg™ connections enable you to create and manage Bloomberg™ orders. Terminal Link routes them through the Execution Management System (EMSX), while the Bloomberg™ FIX connection routes them through the FixNet HUB.</p>
...
<p>The following table describes the available order types for each asset class that the Bloomberg&trade; FIX connection supports:</p>

<table class="qc-table table" id="fix-order-types-table">
   <thead>
      <tr>
        <th>Order Type</th>
        <th>Equity</th>
...
#fix-order-types-table td:not(:first-child),
#fix-order-types-table th:not(:first-child) {
    text-align: center;
}
</style>
...
<p>The Bloomberg&trade; FIX connection does support order updates. The FixNet HUB accepts cancel/replace requests (FIX tag 35=G), so you can update the quantity, limit price, stop price, order type, and time in force of an open order.</p>
