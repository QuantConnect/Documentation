<p>
<? if ($cloudPlatform || $localPlatform) { ?>The live trading results page has a <span class='button-name'>Stop</span> button to immediately stop your algorithm from executing.
<? } elseif ($leanCliCloud) { ?>The <code>lean cloud live stop</code> command immediately stops your algorithm from executing. 
<? } elseif ($leanCli) { ?>The <code>lean live stop</code> command immediately stops your algorithm from executing. <? } ?>
When you stop a live algorithm, your portfolio holdings are retained. Stop your algorithm if you want to perform any of the following actions:
</p>
