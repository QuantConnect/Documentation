<?php
$getHistoricalDataText = function($createSubscriptionsLink, $assetClass, $underlyingSymbolVariable, $underlyingAssetClass, $supportsTicks = false, $contractVariablePy="contract_symbol", $contractVariableC="contractSymbol")
{
    echo "
...
<p>Before you request historical data, call the <code>SetStartDate</code> method with a <code class='python'>datetime</code><code class='csharp'>DateTime </code> to reduce the risk of <a href='/docs/v2/writing-algorithms/getting-started/glossary#16-look-ahead-bias'>look-ahead bias</a>.</p>
...
<p>The preceding calls return the most recent bars, excluding periods of time when the exchange was closed.</p>
    ...
    {
        echo "<p>To get historical data for the continous Futures contract, in the preceding history requests, replace <code class='python'>{$contractVariablePy}</code><code class='csharp'>{$contractVariableC}</code> with <code>future.Symbol</code>.</p>";
    }
