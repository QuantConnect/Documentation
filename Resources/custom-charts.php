...
<p>The candlestick plots that the charting API supports are automatically generated through a stream of values. It's not currently possible to create custom candlestick plots on the results page with your own open, high, low, and close values, but we hope to add the functionality in the future. For the time being, we recommend that you save the plot data in the ObjectStore and then load it into the Research Environment where you can create candlestick charts with <a href="<?= DOCS_ROOT_URL ?>/research-environment/tutorials/plotting-in-research">third-party charting packages</a>.</p>
