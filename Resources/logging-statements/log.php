...
$getLogText = function($isOnOurPlatform) {
    ...
    $result .= "og length is <a href='/docs/v2/our-platform/organizations/resources#09-Log-Quotas'>capped by organization tier</a>. If your organization hits the daily limit, <a href=\"/contact\">contact us</a>.</p> <p>If you log the same content multiple times, only the first instance is added to the log file. To bypass this rate-limiting, add a timestamp to your log messages.</p> <p>To record the algorithm state when the algorithm stops executing, add log statements to the <code>OnEndOfAlgorithm</code> event handler.</p>";
