...
$getLogText = function($isOnOurPlatform) {
    ...
    $result .= "og length is <a href='/docs/v2/cloud-platform/organizations/resources#09-Log-Quotas'>capped by organization tier</a>. If your organization hits the daily limit, <a href=\"/contact\">contact us</a>. The log files of each cloud project can store up to 100,000 lines for up to one year. If you log more than 100,000 lines or some lines become older than one year, we remove the oldest lines in the files so your project stays within the quota.</p> <p>If you log the same content multiple times, only the first instance is added to the log file. To bypass this rate-limiting, add a timestamp to your log messages.</p> <p>To record the algorithm state when the algorithm stops executing, add log statements to the <code>OnEndOfAlgorithm</code> event handler.</p>";
