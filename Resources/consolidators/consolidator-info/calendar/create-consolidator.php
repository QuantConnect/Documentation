...
# self._consolidator = self.create_consolidator(Calendar.WEEKLY, <?=$typeOf?><?=$this->createConsolidatorExtraArgsPy?>)</pre>
	...
	<p>If you need something more specific than the preceding time periods, define a method to set the start time and period of the consolidated bars. The method should receive a <code class='python'>datetime</code><code class='csharp'>DateTime</code> object that's based in the <a href='/docs/v2/writing-algorithms/key-concepts/time-modeling/time-zones#03-Exchange-Time-Zone'>exchange time zone</a> and should return a <code>CalendarInfo</code> object, which contains the start time of the bar in the exchange time zone and the duration of the consolidation period. The following example demonstrates how to create a custom consolidator for weekly bars:</p>
