...
<ul>
    ...
    <? if ($cloud) { ?><li><a href='/docs/v2/cloud-platform/organizations/credit'>QuantConnect Credit (QCC)</a> in your organization.</li><? } ?>
</ul>

<p>Follow these steps to optimize parameters:</p>

<ol>
    <? if ($cloud) { ?><li><a href="/docs/v2/cloud-platform/projects/getting-started#02-View-All-Projects">Open the project</a> that contains the parameters you want to optimize.</li><? } ?>
    <li>In the top-right corner of the IDE, click the <img class='inline-icon' <? if ($cloud) { ?>src='https://cdn.quantconnect.com/i/tu/optimize-parameters-icon.png' alt="Optimization icon"<? } else { ?>src='https://cdn.quantconnect.com/i/tu/optimize-local.svg' alt="Local optimization icon"<? } ?>><span class='icon-name'>Optimize</span> icon.</li>
    ...
    <? if ($cloud) { ?><p>You can optimize a maximum of two parameters. To optimize more parameters, <a href='/docs/v2/lean-cli/optimization/deployment#02-Run-Local-Optimizations'>run local optimizations</a>.</p><? } ?>
    ...
    <? if ($cloud) { ?>
        <li>In the <span class='page-section-name'>Estimated Number<? if ($cloud) { ?> and Cost<? } ?> of Backtests</span>, click an <a href='/docs/v2/cloud-platform/optimization/deployment#02-Resources'>optimization node</a>.</li>
        <li>Select a maximum number of nodes to use.</li>
    <? } ?>
    
    <li>In the <span class='page-section-name'>Estimated Number<? if ($cloud) { ?> and Cost<? } ?> of Backtests</span>, click the <span class='field-name'>Choose Optimization Strategy</span> field and then select a <a href='/docs/v2/<? if ($cloud) { ?>cloud<? } else { ?>local<? } ?>-platform/optimization/strategies'>strategy</a> from the drop-down menu.</li>
    ...
    <p>The <a href='/docs/v2/<? if ($cloud) { ?>cloud<? } else { ?>local<? } ?>-platform/optimization/results#02-View-Optimization-Results'>optimization results page</a> displays. As the optimization job runs, you can close or refresh the window without interrupting the job because the nodes are processing on our servers.</p>
