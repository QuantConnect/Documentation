...
$backtestResultLink = $cloudPlatform ? "/docs/v2/cloud-platform/backtesting/results" : "/docs/v2/local-platform/backtesting/results";
$optimizationResultLink = $cloudPlatform ? "/docs/v2/cloud-platform/optimization/results" : "/docs/v2/local-platform/optimization/results";
?>
...
<ol>
    ...
    <li>In the top-right corner of the IDE, click the <? if ($localPlatform) {?><img class='inline-icon' src='https://cdn.quantconnect.com/i/tu/local-platform-backtest-results-icon.png'> / <?php } ?><img class='inline-icon' src='https://cdn.quantconnect.com/i/tu/cloud-platform-backtest-results-icon.png'> <span class="icon-name">Backtest Results</span> icon.<br></li>
    <p>A table containing all of the backtest results for the project is displayed. If there is a <span class="icon-name">play</span> icon to the left of the name, it's a <a href="<?=$backtestResultLink?>">backtest result</a>. If there is a <span class="icon-name">fast-forward</span> icon next to the name, it's an <a href="<?=$optimizationResultLink?>">optimization result</a>. <? if ($localPlatform) {?>The Platform column displays the <a href='m/docs/v2/local-platform/key-concepts/deployment-targets'>deployment target</a> of the backtest.<? } ?></p>
