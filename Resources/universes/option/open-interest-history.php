...
<div class="section-example-container">
    <pre class="csharp">var history = qb.History&lt;OpenInterest&gt;(<?=$variableNameC?>, TimeSpan.FromDays(3));
foreach (var openInterest in history)
{
    Console.WriteLine(openInterest);
}</pre>
    <pre class="python"># DataFrame format
history_df = qb.history(OpenInterest, <?=$variableNamePy?>, timedelta(3))
display(history_df)

# OpenInterest objects
history = qb.history[OpenInterest](<?=$variableNamePy?>, timedelta(3))
