...
<div class="section-example-container">
    <pre class="csharp">var history = qb.History&lt;TradeBar&gt;(<?=$variableNameC?>, TimeSpan.FromDays(3));
foreach (var tradeBar in history)
{
    Console.WriteLine(tradeBar);
}</pre>
    <pre class="python"># DataFrame format
history_df = qb.history(TradeBar, <?=$variableNamePy?>, timedelta(3))
display(history_df)

# TradeBar objects
history = qb.history[TradeBar](<?=$variableNamePy?>, timedelta(3))
