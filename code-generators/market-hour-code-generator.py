...
'''
            ...
            if "holidays" not in entry or not entry["holidays"]:
                holiday_html += f'<p>There is no holidays for the {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market.</p>'.replace("US Option", "Equity Option").replace("IndexOption", "Index Option")
...
<tr>'''
                    ...
                    holiday_html += f'<p>There are no holidays for the {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market.</p>'.replace("US Option", "Equity Option").replace("IndexOption", "Index Option")
...
<p>The {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market trades in the <code>{', '.join(set([x["exchangeTimeZone"].replace("_", " ") for x in all_entries])) if asset_class == "Index" or asset_class == "IndexOption" else entry["exchangeTimeZone"].replace("_", " ")}</code> time zone.</p>'''.replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))
...
{hour_html}""")
            ...
            if ("[*]" in symbol or "Generic" in symbol) and (sec != ["[*]"] and sec != ["Generic"]):
                with open(symbol_path / 'introduction.html', 'w', encoding='utf-8') as html_file:
                    html_file.write(f"""<p>This page shows the trading hours, holidays, time zone, and supported assets of the {exchange.replace("usa", "us").upper().replace("INDIA", "India") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market.</p>""".replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))

            else:            
                with open(symbol_path / 'introduction.html', 'w', encoding='utf-8') as html_file:
                    html_file.write(f"""<p>This page shows the trading hours, holidays, and time zone of the {contracts_real[symbol] + " (" + symbol +") contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market.</p>""".replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))
