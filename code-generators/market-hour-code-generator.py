...
<p>The {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market trades in the <code>{entry["exchangeTimeZone"].replace("_", " ")}</code> time zone.</p>'''.replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))
