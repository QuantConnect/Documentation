...
from _code_generation_helpers import SPDB, MHDB, get_json_content, MARKET_HOUR
...
''' % exchange.upper()
        ...
        for symbol in sec:
            ...
            if asset_class.lower() != "cfd" and asset_class.lower() != "forex" and symbol != "[*]" and  symbol != "Generic":
...
'''
            ...
            if "holidays" not in entry or not entry["holidays"]:
                holiday_html += f'<p>There is no holidays for the {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India").replace("KRAKEN", "Kraken").replace("BINANCE", "Binance") + " " if asset_class.lower() != "cfd" and asset_class.lower() != "forex" else ""}{asset_class.replace("Cfd", "CFD")} market.</p>'.replace("US Option", "Equity Option").replace("IndexOption", "Index Option")
...
'''
            ...
            with open(symbol_path / f"{MARKET_HOUR.TIME_ZONE}.html", "w", encoding="utf-8") as html_file:
...
<p>The {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India").replace("KRAKEN", "Kraken").replace("BINANCE", "Binance") + " " if asset_class.lower() != "cfd" and asset_class.lower() != "forex" else ""}{asset_class.replace("Cfd", "CFD")} market trades in the {"<code>" + list(all_time_zone)[0] + "</code> time zone." if len(all_time_zone) == 1 else "following time zones:"}</p>
{f"""<ul>
...
'''.replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))
            ...
            for page_name, hour_list in {MARKET_HOUR.PRE_MARKET: premarket_html, MARKET_HOUR.REGULAR: market_html, MARKET_HOUR.POST_MARKET: postmarket_html}.items():
                if all([x == [] for x in hour_list.values()]):
                    with open(symbol_path / f"{page_name}.html", "w", encoding="utf-8") as html_file:
                        html_file.write(f'''<!-- Code generated by market-hour-code-generator.py -->

<p>{"Pre-market" if page_name == MARKET_HOUR.PRE_MARKET else "Post-market"} trading is not available.</p>''')
...
<p>The following table shows the {"pre-market" if page_name == MARKET_HOUR.PRE_MARKET else "post-market" if page_name == MARKET_HOUR.POST_MARKET else "regular" if asset_class == "Index" else "regular trading"} hours for the {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India").replace("KRAKEN", "Kraken").replace("BINANCE", "Binance") + " " if asset_class.lower() != "cfd" and asset_class.lower() != "forex" else ""}{f'{asset_class.replace("Cfd", "CFD")} market' if asset_class != "Index" else "Indices"}:</p>
...
''')
            ...
            with open(symbol_path / f"{MARKET_HOUR.HOLIDAY}.html", "w", encoding="utf-8") as html_file:
...
{holiday_html}""")
            ...
            for page_name, hour_html in {MARKET_HOUR.EARLY_CLOSE: early_close_html, MARKET_HOUR.LATE_OPEN: late_open_html}.items():
                with open(symbol_path / f"{page_name}.html", "w", encoding="utf-8") as html_file:
                    html_file.write("<!-- Code generated by market-hour-code-generator.py -->")
                    
                    if "table" in hour_html:
                        html_file.write(f"""
<p>The following table shows the {page_name.replace("-", " ")} for the {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India").replace("KRAKEN", "Kraken").replace("BINANCE", "Binance") + " " if asset_class.lower() != "cfd" and asset_class.lower() != "forex" else ""}{asset_class.replace("Cfd", "CFD")} market:</p>

{hour_html}""".replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))
...
{hour_html}""")
            ...
            if ("[*]" in symbol or "Generic" in symbol) and (sec != ["[*]"] and sec != ["Generic"]):
                with open(symbol_path / f'{MARKET_HOUR.INTRODUCTION}.html', 'w', encoding='utf-8') as html_file:
                    html_file.write(f"""<p>This page shows the trading hours, holidays, time zone, and supported assets of the {exchange.replace("usa", "us").upper().replace("INDIA", "India").replace("KRAKEN", "Kraken").replace("BINANCE", "Binance") + " " if asset_class.lower() != "cfd" and asset_class.lower() != "forex" else ""}{asset_class.replace("Cfd", "CFD")} market.</p>""".replace("US Option", "Equity Option").replace("IndexOption", "Index Option"))
...
for asset_class in ["Index", "IndexOption"]:
    with open(destination_folder / asset_class.lower() / "usa" / "generic"/ f"{MARKET_HOUR.REGULAR}.html", "w", encoding="utf-8") as html_file:
        exchange = "usa"
...
with open(ice_path / f'{MARKET_HOUR.INTRODUCTION}.html', 'w', encoding='utf-8') as html_file:
