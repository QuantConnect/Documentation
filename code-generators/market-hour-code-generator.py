...
'''
            ...
            with open(symbol_path / 'introduction.html', 'w', encoding='utf-8') as html_file:
                html_file.write(f"""<p>This page explains the market opening dates, hours and holidays of {contracts_real[symbol] + " (" + symbol +") traded in " if "[*]" not in symbol else ""}{exchange.upper()} {asset_class} market.""")
            ...
            if "holidays" not in entry or not entry["holidays"]:
                holiday_html += f'<p>There is no holidays for {contracts_real[symbol] + " in " if "[*]" not in symbol else ""}{exchange.upper()} {asset_class} market.</p>'
...
<p>The {contracts_real[symbol] + " in " if "[*]" not in symbol else ""}{exchange.upper()} {asset_class} is traded in the <code>{entry["exchangeTimeZone"].replace("_", " ")}</code> time zone.</p>''')
...
<p>The following table shows the {"pre-market" if page_name == "pre-market-hours" else "post-market" if page_name == "post-market-hours" else "regular" if asset_class == "Index" else "regular trading"} hours for the {contracts_real[symbol] + " in " if "[*]" not in symbol else ""}{exchange.upper()} {asset_class}{"" if asset_class == "Index" else " market"}:</p>
...
''')
            ...
            with open(symbol_path / "holidays.html", "w", encoding="utf-8") as html_file:
                ...
                if "table" in holiday_html and asset_class != "Forex":
                    html_file.write(f"""
<p>The following table shows the dates of holidays for the {contracts_real[symbol] + " in " if "[*]" not in symbol else ""}{exchange.upper()} {asset_class} market.</p>
            ...
            for page_name, hour_html in {"early-closes": early_close_html, "late-opens": late_open_html}.items():
                with open(symbol_path / f"{page_name}.html", "w", encoding="utf-8") as html_file:
                    ...
                    if "table" in hour_html and asset_class != "Forex":
                        html_file.write(f"""
<p>The following table shows the {page_name.replace("-", " ")} for the {contracts_real[symbol] + " in " if "[*]" not in symbol else ""}{exchange.upper()} {asset_class} market.</p>
