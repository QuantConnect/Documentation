...
<p>The {contracts_real[symbol] + " contract in the " if "[*]" not in symbol else ""}{exchange.replace("usa", "us").upper().replace("INDIA", "India").replace("KRAKEN", "Kraken").replace("BINANCE", "Binance") + " " if asset_class.lower() != "cfd" else ""}{asset_class.replace("Cfd", "CFD")} market trades in the following time zone(s).</p>
<ul>
    <li><code>{"""</code></li>
    <li><code>""".join(set([x["exchangeTimeZone"].replace("_", " ") for x in all_entries])) if asset_class == "Index" or asset_class == "IndexOption" else entry["exchangeTimeZone"].replace("_", " ")}</code></li>
</ul>
