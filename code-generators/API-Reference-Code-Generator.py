...
def ResponseTable(requestBody, type="application/json"):
    writeUp = ""
    array = False
    order = 0
    
    if "content" in requestBody:
        component = requestBody["content"].get(type)
        if not component:
            return ''
    ...
    for api_call, result in paths.items():
        ...
        with open(destination_folder / f'{j:02} Introduction.html', "w") as html_file:
            ...
            if summary and summary[-1] != '.':
                summary += '.'
            html_file.write(f"""<!-- Code generated by API-Reference-Code-Generator.py -->
...
""")
        ...
        if "description" in content:
            with open(destination_folder / f'{j:02} Description.html', "w") as html_file:
                html_file.write("""<!-- Code generated by API-Reference-Code-Generator.py -->
...
""")
        ...
        with open(destination_folder / f'{j:02} Request.html', "w") as html_file:
            ...
            if "parameters" in content:
                ...
                form_data = ResponseTable(content["requestBody"], "multipart/form-data")
                if form_data:
                    writeUp += f'<p>The <code>{api_call}</code> API requires a file request in the following format:</p>'
                    writeUp += form_data
