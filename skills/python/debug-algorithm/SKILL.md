...
- KeyNotFound / "wasn't found in the DataDictionary": find WHY the key is
  absent first. Custom/alt data (`add_data`) has its OWN symbol
...
- None / null price: `seed_initial_prices` only looks
...
1. Selection too strict OR too loose (the usual flat equity / silent-fail
   ...
   - Too loose: the filter passes a very large set (e.g. 1000+) into an
     equal-weight `set_holdings`; LEAN cannot size that many
...
2. Schedule / frequency mismatch:
   ...
   - Rebalance reads `self._universe.selected` on a
...
5. Indicators never ready: `automatic_indicator_warm_up = True`
...
- Alt-data fields (headline, body, sentiment, etc.) come from that dataset's
  page, never from `getattr` guessing.
...
- Fundamental data missing: `np.isnan(value)`,
  never `!= 0`.
...
4. Alt/custom data keyed by the `add_data` symbol; presence checks
