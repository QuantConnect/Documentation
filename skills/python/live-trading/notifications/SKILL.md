...
Commands give you authenticated delivery with real-time pickup — no upload-to-Object-Store-and-poll round-trip — and both REST/API broadcast and project-targeted send (via `project_id` + `link().download_data()`) work for parent/child coordination.
