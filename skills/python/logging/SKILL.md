...
- **`log` right after `market_order`** — context detached from the order. Put it on the order via `tag="..."` instead.
...
7. If it's bulk structured data for Research, is it accumulated in memory and saved to the Object Store in `on_end_of_algorithm`?
8. If it's verbose live-only diagnostics, is it routed through `safe_log` (gated on `self.live_mode`)?
