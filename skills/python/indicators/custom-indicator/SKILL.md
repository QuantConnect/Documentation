...
Ask the user: (1) indicator name; (2) formula/logic; (3) input type -- `IndicatorDataPoint` (single price), `TradeBar` (OHLCV, default), `QuoteBar` (bid/ask); (4) period; (5) internal LEAN indicators (SMA, EMA, RSI) -- construct and update inside `update`; (6) scope -- single symbol or universe; (7) warm-up -- manual (recommended) or automatic.
...
2. Confirm `is_ready` becomes true and at least one order placed (if the algorithm has entry conditions).
