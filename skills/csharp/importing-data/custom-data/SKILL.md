...
- JSON: use `using Newtonsoft.Json.Linq;`, parse named fields, and fail loudly on unexpected shape.
...
5. If data loads but no orders appear, add one temporary event-level log in `OnData`, adjust the strategy condition, then remove noisy logs.
