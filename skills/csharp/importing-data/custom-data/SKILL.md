...
- ZIP: when a `.zip` file contains CSV rows, subscribe to the `.zip` key directly with `FileFormat.Csv`; LEAN unpacks the stream before calling `Reader`, so the parser is the same as the regular CSV example. `FileFormat.ZipEntryName` is for zip entry names, not parsing row content.
...
- ZIP: when the `.zip` is already in Object Store, do not re-upload it, extract it to another Object Store key, or change the original key. Point `SubscriptionDataSource` at the `.zip` key and set `FileFormat.Csv` if the contents are CSV; keep the same `Reader` implementation as a normal CSV file. Use `FileFormat.ZipEntryName` only when the data points are zip entry names.
```csharp
public class MyCustomData : BaseData
{
    public override SubscriptionDataSource GetSource(SubscriptionDataConfig config, DateTime date, bool isLiveMode)
