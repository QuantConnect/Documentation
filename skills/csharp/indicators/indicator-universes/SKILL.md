...
- **Using a short-circuit operator (`&&`) instead of `&` between two update calls.** The short-circuit form means if the first indicator isn't ready, the second never updates that bar, and the two indicators become permanently out of sync. The canonical pattern combines the side-effecting update with the joint readiness check in one expression: `return ind1.Update(...) & ind2.Update(...)`.
