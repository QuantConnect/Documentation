...
- If the ranking AND weighting inputs are obtainable there (the data point's attributes, or `History()` in the universe selection function), finish the job: slice to the traded long/short names, weight them, store `_weightBySymbol`, and return only those names. The rebalance function then just orders the `Dictionary`.
- If finalizing the weights or signal needs data with NO universe selection (e.g. the weighting depends on the SEC reports dataset, readable only after subscribing), you can't compute it in the universe selection function. Select a BOUNDED candidate set there from what IS available, return it (so QC subscribes only to those), then read the extra dataset and compute the final weights in the rebalance function / `OnData`.

**Anti-pattern:** returning the whole screened universe from the universe selection function and ranking off `_universe.Selected` in the rebalance — that subscribes QC to every screened name. The fix is NOT "never compute in the rebalance" (sometimes you must) — it's "never subscribe to more than a bounded set."
```csharp
...
- `History()` in the universe selection function is a HISTORY REQUEST, not a warm-up: it returns pre-start data, so the first rebalance trades immediately. Do NOT `SetWarmUp` for it — warm-up is only for streaming indicators that accumulate state bar-by-bar.
- **Checking whether a name has *enough* history (a "≥ N days of history" / listing-age / seasoning screen) does NOT need a `History()` request.** Pulling N daily bars for every one of thousands of screened candidates just to count them stalls the algorithm. Instead read `f.SecurityReference.IPODate` (a `DateTime` already on the Fundamental data point) and keep names listed long enough: `(Time.Date - f.SecurityReference.IPODate.Date).Days >= N*7/5` (N trading days ≈ N×7/5 calendar days; 126 ≈ ~6 months). (Other Fundamental attributes: the `fundamental-universes` skill.)
