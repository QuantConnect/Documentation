...
Universe mechanics shared by every dynamic universe — the MINUTE resolution default, scheduling selection to the rebalance calendar, and what to return during warm-up — are in the **universes** skill; apply them here too. <!-- python-only -->One cross-sectional-specific addition: when a numeric signal is computed for hundreds or thousands of names every selection (rolling-window arithmetic, z-scores, rank transforms), compute it as array math over the whole cross-section — one names×window matrix, one `argsort` — not a per-name Python loop; at that scale the interpreted loop dominates runtime even with a bounded subscription set. A per-name loop is fine for small universes or genuinely path-dependent per-name logic that array operations cannot express.<!-- /python-only -->
