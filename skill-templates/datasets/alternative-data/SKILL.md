...
| `QuiverInsiderTrading` | Insider Trading | py`date`cs`Date`, py`file_date`cs`FileDate`, py`transaction_code`cs`TransactionCode`, py`price_per_share`cs`PricePerShare`, py`shares`cs`Shares`, py`shares_owned_following`cs`SharesOwnedFollowing`, py`acquired_disposed_code`cs`AcquiredDisposedCode`, py`direct_or_indirect_ownership`cs`DirectOrIndirectOwnership`, py`name`cs`Name`, py`officer_title`cs`OfficerTitle`, py`is_director`cs`IsDirector`, py`is_officer`cs`IsOfficer`, py`is_ten_percent_owner`cs`IsTenPercentOwner`, py`is_other`cs`IsOther` |
| `SpectralTickFlowSignal` | Spectral Tick Flow Signal | py`trade_count`cs`TradeCount`, py`volume_variance_explained`cs`VolumeVarianceExplained`, py`volume_dominant_period_seconds`cs`VolumeDominantPeriodSeconds`, py`execution_score`cs`ExecutionScore`, py`execution_period_seconds`cs`ExecutionPeriodSeconds`, py`execution_size`cs`ExecutionSize`, py`execution_signed`cs`ExecutionSigned`, py`execution_rayleigh`cs`ExecutionRayleigh`, py`signature_count`cs`SignatureCount` |
...
| `BLSEconomicSurveysCes` | US Bureau of Labor Statistics (BLS) | py`total_nonfarm`cs`TotalNonfarm`, py`total_private`cs`TotalPrivate`, py`average_hourly_earnings`cs`AverageHourlyEarnings`, py`average_weekly_hours`cs`AverageWeeklyHours`, py`average_weekly_earnings`cs`AverageWeeklyEarnings`, py`production_hourly_earnings`cs`ProductionHourlyEarnings`, py`production_employees`cs`ProductionEmployees`, py`manufacturing`cs`Manufacturing`, py`goods_producing`cs`GoodsProducing`, py`private_service_providing`cs`PrivateServiceProviding`, py`construction`cs`Construction`, py`retail_trade`cs`RetailTrade`, py`financial_activities`cs`FinancialActivities`, py`education_and_health_services`cs`EducationAndHealthServices`, py`leisure_and_hospitality`cs`LeisureAndHospitality`, py`mining_and_logging`cs`MiningAndLogging` |
| `BLSEconomicSurveysCpi` | US Bureau of Labor Statistics (BLS) | py`all_items`cs`AllItems`, py`core_cpi`cs`CoreCpi`, py`food`cs`Food`, py`food_at_home`cs`FoodAtHome`, py`food_away_from_home`cs`FoodAwayFromHome`, py`energy`cs`Energy`, py`shelter`cs`Shelter`, py`rent_of_primary_residence`cs`RentOfPrimaryResidence`, py`gasoline`cs`Gasoline`, py`medical_care`cs`MedicalCare`, py`apparel`cs`Apparel`, py`education_and_communication`cs`EducationAndCommunication`, py`new_vehicles`cs`NewVehicles`, py`used_cars_and_trucks`cs`UsedCarsAndTrucks`, py`college_tuition_and_fees`cs`CollegeTuitionAndFees` |
| `BLSEconomicSurveysJolts` | US Bureau of Labor Statistics (BLS) | py`job_openings`cs`JobOpenings`, py`job_openings_rate`cs`JobOpeningsRate`, py`hires`cs`Hires`, py`hires_rate`cs`HiresRate`, py`quits`cs`Quits`, py`quits_rate`cs`QuitsRate`, py`total_separations`cs`TotalSeparations`, py`layoffs_and_discharges`cs`LayoffsAndDischarges` |
| `BLSEconomicSurveysPpi` | US Bureau of Labor Statistics (BLS) | py`final_demand`cs`FinalDemand`, py`core_ppi`cs`CorePpi`, py`final_demand_less_food_energy_trade`cs`FinalDemandLessFoodEnergyTrade`, py`final_demand_goods`cs`FinalDemandGoods`, py`final_demand_services`cs`FinalDemandServices`, py`final_demand_construction`cs`FinalDemandConstruction`, py`all_commodities`cs`AllCommodities`, py`farm_products`cs`FarmProducts`, py`processed_foods_and_feeds`cs`ProcessedFoodsAndFeeds`, py`crude_petroleum`cs`CrudePetroleum`, py`final_demand_goods_less_foods`cs`FinalDemandGoodsLessFoods` |
...
| Class | Dataset | Properties |
| --- | --- | --- |
| `QuiverCNBCs`->`QuiverCNBC` | CNBC Trading | py`notes`cs`Notes`, py`direction`cs`Direction`, py`traders`cs`Traders`, py`advice_date`cs`AdviceDate` |
