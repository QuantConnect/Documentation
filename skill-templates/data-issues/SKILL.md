...
Create a Jupyter notebook in the project and pull the asset's history around the suspect date with QuantBook (use the py`add_*`cs`Add*` for the asset's class):
...
Block it by its stable **Security ID** (py`str(symbol.id)`cs`symbol.ID.ToString()`), not its ticker — a ticker can later be reused for a different asset, the Security ID will not:
