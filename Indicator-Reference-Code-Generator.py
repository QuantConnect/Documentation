...
for full, short in dict(sorted(names.items())).items():
    ...
    source = urlopen(f"https://raw.githubusercontent.com/QuantConnect/Lean/master/Indicators/{'Stochastics' if full == 'Stochastic' else full}.cs").read().decode("utf-8")
    lines = source.split("\n")
    for i in range(len(lines)):
        if f"public {full}" in lines[i]:
            j = i - 1
            temp = {"summary": "", "param": []}
            ...
            while "///" in lines[j]:
                if "</param>" in lines[j]:
                    ...
                    temp["param"].append(param)
                    
                elif "</summary>" in lines[j]:
                    temp["summary"] = lines[j-1].split("/// ")[-1]
                    
                j -= 1
            
            temp["param"] = temp["param"].reverse()
            
            constructor_args = lines[i].split("(")[1].split(")")[0].split(",")
            temp["args"] = {x[0]: x[1] for x in [k.split(" ") for k in constructor_args]}
            
            if full in full_apis and len(full_apis[full]) > 0:
