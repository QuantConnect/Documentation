...
<p><code>QCAlgorithm</code> provides a shortcut method for the <code>{full}</code> indicator -- <code>{short}()</code>. It creates an <code>Indicator</code> object, hooks it up for automatic updates, and returns it to be used in your algorithm.</p>

<p>You can determine the specific requirements of the indicator from the reference table below.</p>

{"".join(api)}
<br/>
<p>The indicator resolution can be different from the resolution of your securities data. However, the resolution of the indicator should be equal to or higher than the resolution of your security. In most cases, this usage should be in the <code>Initialize</code> method. If you call this method several times, it will create a new indicator that is not ready to use.</p>
...
<p>In this method, you will recreate the basic indicator usage: create the indicator with its constructor and register the indicator for automatic updates with the <code>RegisterIndicator()</code> method, but could be using a custom resolution via a <code>Consolidator</code>.</p>
...
<h4>Manual Update</h4>
<p>Updating your indicator manually allows you to control which data is used and create indicators of other non-regular price fields. The indicator objects have the <code>Update()</code> method that updates the state of the <code>{full}</code> indicator with the given {update_value}.</p>
