<p>
    ...
    filter. These properties are subsets of the <code>FineFundamental</code> object presented to your universe filter
    each day.
</p>
<style>
    .fundamental-type {
    ...
    .fundamental-table .dummy-reference {
        position: relative;
        top: -100px;
        display: block;
    }
    ...
    <table class="table qc-table fundamental-table table-itemized table-reflow">
        ...
        foreach ($field['properties'] as $property) {
            ...
            if (count($children) == 0) {
                ...
                $htmlId = "{$fieldName}-{$propertyName}";
                $htmlId = strtolower($htmlId);
                $htmlId = preg_replace('/\s+/m', '-', $htmlId);

                ?>
                <tr>
                    <td class="fundamental-type">
                        <span class="dummy-reference" id="<?= $htmlId ?>"></span><code><?= $propertyName ?></code><br/>
                        <p><?= $type ?></p>
                    </td>
                    <td>
                        <p><?= $description ?></p>
                ...
                foreach ($children as $child) {
                    ...
                    $htmlId = "{$fieldName}-{$propertyName}-{$childName}";
                    $htmlId = strtolower($htmlId);
                    $htmlId = preg_replace('/\s+/m', '-', $htmlId);
                    ...
                    <tr>
                        <td class="fundamental-type">
                            <span class="dummy-reference" id="<?= $htmlId ?>"></span>
                            <code><?= "{$propertyName}.{$childName}"; ?></code><br/>
                            <p><?= $childType; ?></p>
                        </td>
                        <td>
