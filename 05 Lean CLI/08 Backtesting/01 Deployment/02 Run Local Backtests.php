...
<h4>Parameters</h4>
<p>To run a local backtest with specific <a href='https://www.quantconnect.com/docs/v2/writing-algorithms/optimization/parameters'>parameters</a>, use the <code>--parameter name value</code> option.</p>
<div class="cli section-example-container">
<pre>$ lean backtest "My Project" --parameter ema_fast 100 --parameter ema_slow 50
20210322 17:27:46.658 TRACE:: Engine.Main(): LEAN ALGORITHMIC TRADING ENGINE v2.5.0.0 Mode: DEBUG (64bit)
20210322 17:27:46.664 TRACE:: Engine.Main(): Started 5:27 PM
Successfully ran 'My Project' in the 'backtesting' environment and stored the output in 'My Project/backtests/2021-03-22_18-51-28'</pre>
</div>
...
<div class="section-example-container testable">
	<pre class="python"># Buy a SPY call Option contract and hold it until expiry.
class UsEquityOptionsDemoAlgorithm(QCAlgorithm):
    def initialize(self) -&gt; None:
        self.set_start_date(2024, 1, 1)
        self.set_end_date(2024, 3, 1)
        self.set_cash(100000)
        equity = self.add_equity("SPY", Resolution.MINUTE)
        option = self.add_option(equity.symbol, Resolution.MINUTE)
    ...
    def on_data(self, slice: Slice) -&gt; None:
        ...
        for chain in slice.option_chains.values():
            ...
            if not calls:
                continue
            contract = sorted(calls, key=lambda c: abs(c.strike - c.underlying_last_price))[0]
            self.market_order(contract.symbol, 1)
            self._contract = contract.symbol</pre>
	<pre class="csharp">// Buy a SPY call Option contract and hold it until expiry.
public class UsEquityOptionsDemoAlgorithm : QCAlgorithm
{
    ...
    public override void OnData(Slice slice)
    {
        ...
        foreach (var chain in slice.OptionChains.Values)
        {
            var contract = chain
                ...
                .OrderBy(c =&gt; Math.Abs(c.Strike - c.UnderlyingLastPrice))
                .FirstOrDefault();
...
<p>If you have the latest version of LEAN and you get different <a href="https://www.quantconnect.com/docs/v2/cloud-platform/backtesting/results#09-Key-Statistics">overall statistics</a> when you run the algorithm on your local machine versus in the cloud, delete your <a href="/docs/v2/lean-cli/datasets/format-and-storage#02-Default-Location">local data files</a> and <a href="/docs/v2/lean-cli/datasets/quantconnect">re-download them</a>. Some of your local files may be outdated and the preceding download commands didn't update them.</p>
