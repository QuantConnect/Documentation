<p>With automatic updates, your indicators automatically update with the security data on a schedule you set. To configure automatic updates, create a consolidator and then call the <code>RegisterIndicator</code> method. If your algorithm has a dynamic universe, save a reference to the consolidator so you can <a href="/docs/v2/writing-algorithms/consolidating-data/key-concepts#07-Remove-Consolidators">remove it</a> when the universe removes the security. If you register an indicator for automatic updates, don't call the indicator's <code>Update</code> method or else the indicator will receive double updates.</p>
